...
HtaccessInterpreter.prototype.parseFile = function () {
  ...
  function (err, parsedFile) {
    ...
    for (var i = 0; i < parsedFile.RewriteRules.length; i++) {
      ...
      if (acceptRule === 3) {
        this.rules.push(new RewriteRule(rule, {documentRoot: this.documentRoot}));
      }
    }
  }.bind(this));
};
...
HtaccessInterpreter.prototype.middleware = async function (req, res, next) {
  ...
  for (var i = 0; i < this.rules.length; i++) {
    ...
    if (await rule.conditionsMatches(req)) {
      var action = await rule.matches(reqUrl);
      ...
      if (action) {
        if (action.type === 'status') {
          ...
          if (this.verbose) {
            console.log(chalk.green.bgBlack(' Rewriting (' + action.code + ') ' + reqUrl + ' to ' + action.dest));
          }

          if (rule.flags.indexOf('L') != -1) break;
