...
function RewriteRule(baseRule, opts) {
  _.assign(this, baseRule);
  opts = opts || {};

  this.documentRoot = opts.documentRoot;

  this.substitution = this.substitution.replace(/\\(%|\$)/g, '$1');
  ...
  for (var i = 0; i < this.conditions.length; i++) {
    this.conditions[i] = new RewriteCond(this.conditions[i], opts);
  }
...
RewriteRule.prototype.conditionsMatches = async function (req) {
  var state = {
    currentIsOr: false,
    prevWasOr: false,
  ...
  for (var i = 0; i < this.conditions.length; i++) {
    ...
    // condition isn't met
    if (await this.conditions[i].matches(req) === false) {
...
RewriteRule.prototype.matches = async function (url) {
